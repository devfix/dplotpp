...
namespace dplot
{
    ...
    namespace
    {
        ...
        void append(std::vector<std::string> &out, const std::vector<std::string> &in) { out.insert(out.end(), in.begin(), in.end()); }
        ...
        private:
            ...
            const std::vector<std::string> LatexCmdsAfterDocClass = {R"(\usepackage{tikz,amsmath,siunitx})",
            ...
            [[nodiscard]] std::optional<std::pair<double, double>> get_y_domain(const AxisSetup &asy) const
            {
                ...
                double mn = asy.limits->first > 0 ? asy.limits->first / overscale_limit : asy.limits->first * overscale_limit;
                double mx = asy.limits->second > 0 ? asy.limits->second * overscale_limit : asy.limits->second / overscale_limit;
                return std::make_pair(mn, mx);
            }

            [[nodiscard]] std::vector<std::string> get_axis_param(char axis_kind, const std::optional<AxisSetup> &axis_setup, std::optional<std::pair<double, double>> limits = std::nullopt) const
            {
                if (!limits.has_value() && axis_setup.has_value()) { limits = axis_setup->limits; }
                return {"scale only axis", "width=" + fig->width, "height=" + fig->height, std::string(1, axis_kind) + "min=" + fmt_flt(limits->first),
                        std::string(1, axis_kind) + "max=" + fmt_flt(limits->second)};
            }

            [[nodiscard]] std::vector<std::string> create_doc_begin() const
            {
                std::vector<std::string> out = {"%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%", "% auto-generated using dplot %", "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"};
                ...
                out.push_back(R"(\pgfplotsset{every axis/.append style={)" + fig->basic_thickness + R"(},compat=1.18},)");
                return out;
            }
            ...
            [[nodiscard]] std::vector<std::string> create_background() const
            {
                ...
                for (const auto &[axis, axis_setup_opt] : fig->axes)
                {
                    ...
                    params.push_back(std::string(1, axis_kind) + "majorgrids=" + bool_to_str(axis_setup.grid.major_enable));
                    params.push_back("major grid style={" + axis_setup.grid.major_thickness + ",color=" + axis_setup.grid.major_color + "}");
                    params.push_back(std::string(1, axis_kind) + "minorgrids=" + bool_to_str(axis_setup.grid.minor_enable));
                    params.push_back("minor grid style={" + axis_setup.grid.minor_thickness + ",color=" + axis_setup.grid.minor_color + "}");
                    ...
                    params.push_back(std::string(1, axis_kind) + "tick pos=" + (axis_setup.tick.opposite ? "both" : get_axis_pos(axis)));
                    params.push_back(std::string(1, axis_kind) + "tick distance=" + (axis_setup.tick.major_distance ? fmt_flt(axis_setup.tick.major_distance.value()) : ""));
                    params.push_back("major " + std::string(1, axis_kind) + " tick style={" + axis_setup.tick.major_thickness + ",color=" + axis_setup.tick.major_color + "}");
                    params.push_back("minor " + std::string(1, axis_kind) + " tick style={" + axis_setup.tick.minor_thickness + ",color=" + axis_setup.tick.minor_color + "}");
                    params.push_back("minor " + std::string(1, axis_kind) + " tick num=" + std::to_string(axis_setup.tick.minor_num));
            ...
            [[nodiscard]] std::vector<std::string> create_plot_content(char ax, char ay, const Data &data) const
            {
                ...
                std::vector<std::string> params_plot = {"color=" + data.ls.plot_color,
                                                        data.ls.line_style,
                ...
                if (y_domain.has_value()) { params_plot.push_back("restrict y to domain={" + fmt_flt(y_domain->first) + ":" + fmt_flt(y_domain->second) + "}"); }

                std::vector<std::string> params_table = {"row sep=newline", R"(x expr=\thisrowno{0}*)" + fmt_flt(asx.scale), R"(y expr=\thisrowno{1}*)" + fmt_flt(asy.scale)};
                ...
                for (decltype(data.dx)::index_type i = 0; i < data.dx.size(); ++i) { out.push_back("  " + fmt_flt(data.dx[i]) + " " + fmt_flt(data.dy[i])); }
                out.emplace_back("};");
            ...
            {
                std::vector<std::string> out = {"", "%%%%%%%%%%%%%%%%%%", "% plot group " + std::string(1, ax) + "/" + std::string(1, ay) + " %", "%%%%%%%%%%%%%%%%%%"};
            ...
            [[nodiscard]] std::vector<std::string> create_legend() const
            {
                ...
                std::vector<std::string> legend_style = {"at={(" + fmt_flt(fig->legend_setup.at.first) + "," + fmt_flt(fig->legend_setup.at.second) + ")}", "anchor=" + fig->legend_setup.anchor,
                ...
                std::string ls_join;
                for (size_t i = 0; i < legend_style.size(); ++i) { ls_join += legend_style[i] + (i < legend_style.size() - 1 ? ", " : ""); }
                ...
                for (const auto &data : fig->plot_data)
                {
                    ...
                    out.push_back(R"(\addlegendimage{/pgfplots/refstyle=dplot:)" + std::to_string(data._id) + R"(}\addlegendentry{)" + label + "}");
                }
                out.emplace_back(R"(\end{axis})");
                return out;
            }
    ...
    Data::Data(XAxis ax, YAxis ay, TypeData dx, TypeData dy, std::string label, LineSetup ls) : ax(ax), ay(ay), dx(std::move(dx)), dy(std::move(dy)), label(std::move(label)), ls(std::move(ls)) {}
    ...
    Data &Figure::plot(XAxis ax, YAxis ay, const TypeData &dx, const TypeData &dy, std::string label, const std::optional<LineSetup>& ls)
    {
        ...
        for (auto &[ax_key, axis_setup] : axes)
        {
            if (axis_setup && !axis_setup->limits.has_value())
            {
                ...
                for (const auto &data : plot_data)
                {
                    ...
                    nc::NdArray<double> nd_dx(data.dx);
                    nc::NdArray<double> nd_dy(data.dy);

                    if (data_ax_char == ax_key)
                    {
                        mx = std::max(mx, axis_setup->scale * nc::max(nd_dx).item());
                        mn = std::min(mn, axis_setup->scale * nc::min(nd_dx).item());
                    }
                    if (data_ay_char == ax_key)
                    {
                        mx = std::max(mx, axis_setup->scale * nc::max(nd_dy).item());
                        mn = std::min(mn, axis_setup->scale * nc::min(nd_dy).item());
                    }
                }
    ...
    void Figure::show()
    {
        ...
        for (const auto &data : plot_data)
        {
            const auto p = matplot::plot(data.dx.toStlVector(), data.dy.toStlVector());
    ...
    void Figure::export_figure(const std::string &path_out_dir, const std::vector<ExportType> &types)
    {
        ...
        if (need_pdf)
        {
            std::string cmd = Environment::PATH_PDFLATEX + " -interaction=nonstopmode -output-directory=" + out_dir.string() + " " + path_latex.string();
